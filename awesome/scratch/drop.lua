...
function toggle(prog, vert, horiz, width, height, yoffset, xoffset, sticky, screen)
    vert   = vert   or "top"
    ...
    if not dropdown[prog][screen] then
        spawnw = function (c)
            ...
            if     horiz == "left"  then x = screengeom.x + xoffset
            elseif horiz == "right" then x = screengeom.width - width - xoffset
            else   x =  screengeom.x+(screengeom.width-width)/2 + xoffset end

            if     vert == "bottom" then y = screengeom.height + screengeom.y - height - yoffset
            elseif vert == "center" then y = screengeom.y+(screengeom.height-height)/2 + yoffset
            else   y =  yoffset end
